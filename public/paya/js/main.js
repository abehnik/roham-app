$(window).scroll(function () {
  ...
  $('.small-logo,.rotate-icon').css(
    'transform',
    'rotate(' + 2 * body_sc + 'deg)'
  )

  $('.product-text').css('transform', 'translateY(-' + body_sc / 3 + 'px)')
  $('#About,.about-goal').css(
    'background-position',
    '  center calc(50% + ' + $(window).scrollTop() * 0.1 + 'px'
  )
})
...
$(document).ready(function () {
  ...
  options3 = {
    ...
    nav: true,
    navText: [
      '<span  class=" slider-left-btn"> </span>',
      '<span  class=" slider-right-btn"> </span>'
    ],
    navRewind: false,
    dots: true,
    mouseDrag: false,
  ...
  $(function () {
    $('#search-site').focus(function () {
      ...
      if ($(this).val() == Se) $(this).val('')
    })
    $('#search-site').blur(function () {
      if ($(this).val() == '') $(this).val(Se)
    })
    Se = $('#search-site').val()
  })
